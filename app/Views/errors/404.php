...
<div class="error-shell">
    <section class="error-card card-panel">
        <div class="error-kicker"><span class="error-code">404</span><span class="error-label">Missing Page</span></div>
        <div class="error-icon"><i class="bi bi-compass"></i></div>
        <div class="error-heading">
            <h1><?= e($title ?? 'Page not found') ?></h1>
            <p><?= e($message ?? 'The page you requested does not exist or may have moved.') ?></p>
        </div>
        <div class="error-actions">
            <a href="<?= e(url('dashboard')) ?>" class="btn btn-primary">Back to dashboard</a>
            <button type="button" class="btn btn-outline-secondary" onclick="window.history.back()">Previous page</button>
        </div>
        <div class="error-note">Check the URL or return to the dashboard to continue working.</div>
    </section>
</div>
</body>
</html>
