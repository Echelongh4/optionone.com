...
<div class="error-shell">
    <section class="error-card card-panel">
        <div class="error-kicker"><span class="error-code">403</span><span class="error-label">Permission Required</span></div>
        <div class="error-icon"><i class="bi bi-shield-lock"></i></div>
        <div class="error-heading">
            <h1><?= e($title ?? 'Access denied') ?></h1>
            <p><?= e($message ?? 'You do not have permission to access this page.') ?></p>
        </div>
        <div class="error-actions">
            <a href="<?= e(url('dashboard')) ?>" class="btn btn-primary">Back to dashboard</a>
            <button type="button" class="btn btn-outline-secondary" onclick="window.history.back()">Go back</button>
        </div>
        <div class="error-note">If you believe this is a mistake, contact an administrator to review your access level.</div>
    </section>
</div>
</body>
</html>
